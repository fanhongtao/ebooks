...
\begin{figure}[H]
    ...
    \begin{minipage}{8cm}
        \centering
        \input{../pic/ds2-ch3-3-4-1}
        \caption*{$|x| < a$ \\ (1)}
    \end{minipage}
    \quad
    \begin{minipage}{8cm}
        \centering
        \input{../pic/ds2-ch3-3-4-2}
        \caption*{$|x| > a$ \\ (2)}
    \end{minipage}
...
\xiaoti{设 $\epsilon > 0$，解不等式 $|x - A| < \epsilon$，并且在数轴上表示出它的解集。}


\xiaoti{已知 $|A - a| < \dfrac{\epsilon}{2}$，$|B - b| < \dfrac{\epsilon}{2}$，求证：}
\begin{xiaoxiaotis}

    \xiaoxiaoti{$|(A + B) - (a + b)| < \epsilon$；}
